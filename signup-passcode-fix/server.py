...
# ============= SIGNUP PASSCODE ROUTES =============

# Default signup passcode
DEFAULT_SIGNUP_PASSCODE = "1234"

@api_router.get("/signup-passcode/verify/{passcode}")
async def verify_signup_passcode(passcode: str):
    """Verify signup passcode (public endpoint - no auth required)"""
    # Get global settings
    global_settings = await db.global_settings.find_one({'key': 'signup_passcode'})
    
    stored_passcode = DEFAULT_SIGNUP_PASSCODE
    if global_settings and global_settings.get('value'):
        stored_passcode = global_settings['value']
...
@api_router.get("/signup-passcode")
async def get_signup_passcode(user: dict = Depends(get_current_user)):
    """Get current signup passcode (admin only)"""
    check_permission(user, Permission.MANAGE_ORGANIZATION)
    
    global_settings = await db.global_settings.find_one({'key': 'signup_passcode'})
    
    if global_settings and global_settings.get('value'):
        return {"passcode": global_settings['value']}
    else:
        return {"passcode": DEFAULT_SIGNUP_PASSCODE}

@api_router.put("/signup-passcode")
async def update_signup_passcode(request: dict, user: dict = Depends(get_current_user)):
    """Update signup passcode (admin only)"""
    check_permission(user, Permission.MANAGE_ORGANIZATION)
    
    new_passcode = request.get('passcode', '')
    
    # Validate passcode is exactly 4 digits
    ...
    return {"message": "Signup passcode updated successfully", "passcode": new_passcode}
